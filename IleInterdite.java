...
class CModele extends Observable {
	...
	//Méthodes sensé vérifier qu'un joueur ne peut pas aller sur une case inondée
	public boolean testCaseAutour(Joueur j, Direction dir) {
		int x = j.getX();
		int y = j.getY();
		switch (dir) {
			case Haut:
				if (!(zones[x][y-1].etat.equals(EtatZone.Innondee))) {
					return true;
				}
				break;
			case Bas:
				if (y + 1 < CModele.HAUTEUR && !(zones[x][y-1].etat.equals(EtatZone.Innondee))) {
					return true;
				}
				break;
			case Gauche:
				if (x - 1 > 0 && !(zones[x][y-1].etat.equals(EtatZone.Innondee))) {
					return true;
				}
				break;
			case Droite:
				if (x + 1 < CModele.LARGEUR && !(zones[x][y-1].etat.equals(EtatZone.Innondee))) {
					return true;
				}
				break;
			default:
	...
	public void deplaceJoueur(Joueur j, Direction dir){
		if(testCaseAutour(j, dir)== true){
			j.deplace(dir);
		}
	}
